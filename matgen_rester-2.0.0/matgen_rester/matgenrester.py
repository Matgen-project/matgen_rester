...
class MatgenRester:
  ...
  def get_structure_with_filter(self,filter=None, size=1, page=0):
    param={
      "pages": size,
      "pageno": page
    }
    r = self._make_request('filter',param=param, payload=filter)
    return r
...
if __name__ == "__main__":
  ...
  if parser.id:
    context = matgenrester.get_strcuture_by_id(parser.idType, parser.id,fields = fields)
  else:
    filter={}
    ...
    context = matgenrester.get_strcuture_with_filter(filter=filter, size=parser.size, page=parser.index)            
